...
export function createDataAbandonController({ pool }: Deps): Router {
  ...
  router.get("/", async (req: Request, res: Response) => {
    ...
    if (trackingOnly) {
      conditions.push(
        `EXISTS (SELECT 1 FROM tracking_items ti WHERE ti.data_id = data_items_abandon.id)`
      );
    ...
    try {
      ...
      const countResult = await pool.query(
        `SELECT COUNT(*) AS cnt FROM data_items_abandon ${whereClause}`,
        params
      );
      ...
      const dataResult = await pool.query(
        `SELECT
           ...
           url,
           keywords,
           EXISTS (SELECT 1 FROM tracking_items ti WHERE ti.data_id = data_items_abandon.id) AS tracking,
      ...
      console.error("GET /web/data-abandon error", err);
      res.status(500).json({ message: "Failed to load abandon data" });
    }
  });
  ...
  router.post("/track", async (req: Request, res: Response) => {
    ...
    try {
      // 1) 更新 data_items_abandon.tracking
      await pool.query(
        `UPDATE data_items_abandon
           SET tracking = $1,
      ...
      // eslint-disable-next-line no-console
      console.error("POST /web/data-abandon/track error", err);
      res.status(500).json({ message: "Failed to update abandon tracking" });
    }
  });
  ...
  router.post("/read", async (req: Request, res: Response) => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("POST /web/data-abandon/read error", err);
      res.status(500).json({ message: "Failed to update abandon read status" });
    }
  });

  // 将指定 channel 加入黑名单，并把对应数据全部标记为已读。
  // 这里保持与主数据列表一致：写入 reddit_subreddit_blacklist，
  // 并对主表 data_items 做已读标记（而不是 abandon 表）。
  router.post(
    "/channel/blacklist",
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          "POST /web/data-abandon/channel/blacklist error",
          err
        );
        res
          .status(500)
